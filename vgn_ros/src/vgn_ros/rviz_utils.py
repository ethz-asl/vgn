...
class RViz(object):
    def __init__(self, frame="task"):
        self._pubs = dict()
        self._pubs["point_cloud"] = rospy.Publisher(
            "/point_cloud", PointCloud2, queue_size=1
        )
        self._pubs["tsdf"] = rospy.Publisher("/tsdf", PointCloud2, queue_size=1)
        self._pubs["grasp_pose"] = rospy.Publisher(
            "/grasp_pose", PoseStamped, queue_size=1
        )
        self._pubs["candidates"] = rospy.Publisher(
            "/candidates", PointCloud2, queue_size=1
        )
        self._pubs["true_false"] = rospy.Publisher(
            "/true_false", PointCloud2, queue_size=1
        )
    ...
    def draw_candidates(self, poses, scores):
        ...
        msg = ros_utils.to_point_cloud_msg(points, intensities=scores, frame="task")
        self._pubs["candidates"].publish(msg)

    def draw_true_false(self, poses, trues):
        points = np.reshape([p.translation for p in poses], (len(poses), 3))
        trues = np.expand_dims(trues, 1)
        msg = ros_utils.to_point_cloud_msg(points, intensities=trues, frame="task")
